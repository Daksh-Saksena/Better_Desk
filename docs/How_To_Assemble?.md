...
The motors spin the threaded rods and the mechanism transfers that motion to the balls.
...
Mount the motors to the frame and connect them to the threaded rods. both NEMA 13 stepper motors and 28BYJ-48 motors with ULN2003 drivers. Connect the motor drivers and ESP32 boards according to the wiring diagram.

---

# 3. Pneumatic system

Install the mini pumps and route the tubing through the relevant parts of the mechanism.

Connect the pressure sensors to the pneumatic sections and make sure the tubing connections are secure.

Test one complete cell before connecting everything together.
...
Connect the Raspberry Pi, ESP32s and motor drivers according to the wiring diagram.
...
Mount the phone camera and iPad camera so they can see the whole workspace.

The cameras are calibrated together so the software can understand the workspace in 3D.
...
Once the software knows where objects are, those positions can also be used by the projector to show boxes, arrows, labels and instructions.
...
The LCD is taken from an old broken tablet and used without its original backlight.

## 6.1 LED and cooling

Mount the LED onto the aluminium cooling assembly and attach the CPU cooler.

The 50W LED produces a lot of heat, so make sure the cooling system is properly mounted before running it.
...
Use the CAD dimensions for the exact spacing.
...
The control panel is made mostly from old hardware.
...
- TSC2046 touch controller

Mount the touchscreen over the display and connect the touch controller to the Arduino.

The Arduino handles the control panel and communicates with the Raspberry Pi.

---

# 8. Waterproofing

Some parts need to be waterproof, but printing all of them in SLA would be too expensive.
...
The first proper milestone should be one complete working cell. Once that works, scale it up to the full 10x10 surface.
