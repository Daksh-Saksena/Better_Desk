...
const px = (v) => v * W();          // norm X → pixels
const py = (v) => v * H();          // norm Y → pixels
const ps = (v) => v * Math.min(W(), H()); // norm size → pixels (aspect-safe)

// ═══════════════════════════════════════════════════════════════════════════
//  Scene graph
// ═══════════════════════════════════════════════════════════════════════════
const scene = new Map(); // id → node
...
function computeAnimatedOpacity(node, t) {
  ...
  const phase = (t % dur) / dur; // 0 → 1 within one cycle
...
function drawArrow(node, t) {
  ...
  // Animated offset for moving arrows
  let dashOffset = 0;
...
function drawNode(node, t) {
  ...
  // Transform (scale + rotation around node center)
...
function render(now) {
  ...
  // Sort by layer (stable sort — lower layer renders first)
